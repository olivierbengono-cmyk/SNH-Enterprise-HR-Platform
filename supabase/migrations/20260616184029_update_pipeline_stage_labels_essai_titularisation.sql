-- Update notification trigger to use new stage names (Essai / Titularisation)
...
DECLARE
  ...
  v_status_label text;
BEGIN
  SELECT c.user_id INTO v_user_id FROM candidates c WHERE c.id = NEW.candidate_id;
  ...
  ELSIF TG_OP = 'UPDATE' AND (OLD.status IS DISTINCT FROM NEW.status) THEN
    v_status_label := CASE NEW.status
      ...
      WHEN 'pre_onboarding'  THEN 'Période d''essai'
      WHEN 'onboarding'      THEN 'Période d''essai'
